...
class GameField(context: Context?, private val game: Game = Game()) : View(context) {
    ...
    private fun initSliderAnim() {
        ...
        sliderAnimator = ObjectAnimator.ofFloat(game.slider,Slider::X.name,0f, width.toFloat() - slider.width)
        ...
        timer.schedule(object : TimerTask() {
            override fun run() {
                val bub = game.act(height - slider.height.toFloat())
                if (bub != null)
                    addBubbleAnim(bub)
